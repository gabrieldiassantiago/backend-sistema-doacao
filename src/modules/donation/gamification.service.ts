...
export class GamificationEngine {
  private static readonly LEVELS: LevelDef[] = [
  ...
  private static readonly BADGE_META: Record<BadgeKey, BadgeMeta> = {
    FIRST_DONATION: {
      key: "FIRST_DONATION",
      name: "Primeiro Passo",
   ...
   */
  static computeLevel(xpPoints: number): LevelInfo {
   ...
   */
  static computeXpForDonation(amount: number, donationCountAfter: number): number {
  ...
  /**
   * Retorna os metadados de todos os emblemas disponíveis.
   */
  static getBadgeMeta(): Record<BadgeKey, BadgeMeta> {
    return this.BADGE_META;
  }
}

export const LEVELS = GamificationEngine["LEVELS"];
export const BADGE_META = GamificationEngine.getBadgeMeta();
export const computeLevel = (xp: number) => GamificationEngine.computeLevel(xp);
export const computeXpForDonation = (amount: number, count: number) => GamificationEngine.computeXpForDonation(amount, count);
export const computeNewBadges = (count: number, total: number, earned: BadgeKey[]) => GamificationEngine.computeNewBadges(count, total, earned);
